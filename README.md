...
- **Только один экземпляр.** Бот работает через long polling: два процесса с
  ...
  сервис `app`.
- **Если сервер не выделенный.** Когда порт 443 на хосте уже кем-то занят —
  например `tailscale serve` держит его на адресе тайлнета — Caddy не стартует
  с `address already in use`: привязка к `0.0.0.0` конфликтует с любой
  существующей привязкой на том же порту. Лечится переменной `BIND_IP` в
  `.env`: задайте адрес интерфейса самого инстанса (`ip -4 addr show scope
  global`), и Caddy займёт 80/443 только на нём. За NAT в Oracle это приватный
  IP, внешний трафик всё равно приходит на него.
