...
## SRS-карточка (расширенный формат)

Новый компонент карточки для интервального повторения по принципам эффективного
запоминания — `web/src/components/srs/SrsCard.tsx` (пока отдельно от основного
потока приложения, с моком на слове *drizzle*):
