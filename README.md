...
Есть тёмная тема в мягких сине-графитовых тонах (переключатель в шапке;
по умолчанию берётся из темы Telegram; цвета тем — CSS-переменные в
`web/src/styles/index.css`). При перетаскивании карточки интерфейс
подкрашивается зелёным («помню», вправо) или красным («забыл», влево) с
крупной подписью действия.
...
изучения и язык перевода выбираются на вкладке «Профиль».
